...
function TimelinePanel({ visibleMessageIds, completedObjectives, selectedKey, onJump }: { visibleMessageIds: Set<string>; completedObjectives: Array<(typeof objectives)[number] & { done: boolean }>; selectedKey: string | null; onJump: (messageId: string) => void }) {
  ...
  return <><h2 className="panel-title">{threadTitle}</h2><div className="panel-subtitle">Timeline mode turns the thread into a readable progress index.</div><div className="panel-timeline-list panel-block">{visibleEvents.map((event) => { const isHighlighted = !!selectedKey && `${event.title} ${event.description} ${event.linkedObjective ?? ''}`.toLowerCase().includes(selectedKey.toLowerCase()); return <button type="button" className={`timeline-card ${isHighlighted ? 'highlighted' : ''}`} key={event.title} onClick={() => onJump(event.sourceMessageId)}><div className="timeline-meta"><span>{event.time}</span>{event.linkedObjective && <span className="timeline-link">{event.linkedObjective}</span>}</div><div className="timeline-title">{event.title}</div><p className="timeline-description">{event.description}</p></button> })}</div><div className={`panel-objectives panel-block ${allDone ? 'compact' : ''}`}>{completedObjectives.map((obj, i) => <div className={`objective-row ${obj.done ? 'done' : ''}`} key={i}>{obj.done ? <CheckIcon /> : <div className="objective-dot" />}{allDone ? <span className="objective-label compact-label">{obj.label}</span> : <span className="objective-label">{obj.label}</span>}</div>)}</div></>
}
